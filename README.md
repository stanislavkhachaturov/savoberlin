...
Сейчас используются лицензионно-свободные фото с Unsplash. **Их стоит заменить на реальные фотографии работ** — особенно пару `vorher` / `nachher`: она подобрана так, чтобы кадры читались как одно помещение (тот же тип комнаты, балконная дверь и радиатор справа, тот же ракурс), но идеально это решается только двумя реальными снимками одной квартиры до и после. Пара кропается строго по центру (`position: "center"`), чтобы кадры совпадали.
...
- [ ] **Цифры** — 12 лет, 3.200 заказов, 4,9/5, 98 % рекомендаций. Это заглушки: их нельзя публиковать непроверенными, в Германии за недостоверную рекламу прилетает от конкурентов (UWG).
